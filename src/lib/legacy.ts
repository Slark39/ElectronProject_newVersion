...
export function isTriggerRatioInverted(fromTokenInfo:any, toTokenInfo:any) {
...
export function shouldInvertTriggerRatio(tokenA:any, tokenB:any) {
...
export function getTargetUsdgAmount(token:any, usdgSupply: bigint, totalTokenWeights:any): bigint | undefined {
...
export function getBuyGlpToAmount(fromAmount:any, swapTokenAddress:any, infoTokens:any, glpPrice:any, usdgSupply:any, totalTokenWeights:any) {
...
export function getSellGlpFromAmount(toAmount:any, swapTokenAddress:any, infoTokens:any, glpPrice:any, usdgSupply:any, totalTokenWeights:any) {
  const defaultValue = { amount: 0n, feeBasisPoints: 0 };
...
export function getProfitPrice(closePrice:any, position:any) {
  let profitPrice;
...
export function getPositionContractKey(account:any, collateralToken:any, indexToken:any, isLong:any) {
...
export function getSwapFeeBasisPoints(isStable:any) {
...
function _parseOrdersData(ordersData:any, account:any, indexes:any, extractor:any, uintPropsLength:any, addressPropsLength:any) {
...
function parseDecreaseOrdersData(chainId:any, decreaseOrdersData:any, account:any, indexes:any) {
...
function parseIncreaseOrdersData(chainId:any, increaseOrdersData:any, account:any, indexes:any) {
  const extractor = (sliced:any) => {
...
function parseSwapOrdersData(chainId:any, swapOrdersData:any, account:any, indexes:any) {
...
export function getOrderKey(order:any) {
...
export function useAccountOrders(
  ...
  const {
    ...
    fetcher: async ([, chainId, orderBookAddress, account]:any) => {
        ...
        return fetch(ordersIndexesUrl)
          .then(async (res) => {
            ...
            for (const key of Object.keys(json)) {
              (ret as any)[key.toLowerCase()] = json[key].map((val:any) => parseInt(val.value)).sort((a:any, b:any) => a - b);
            }
      ...
      const fetchLastIndex = async (type:any) => {
      ...
      const getIndexes = (knownIndexes:any, lastIndex:any) => {
      ...
      const getOrders = async (method:any, knownIndexes:any, lastIndex:any, parseFunc:any) => {
...
export function getAccountUrl(chainId: any, account: any) {
  if (!account) {
...
export function isMobileDevice(navigator: any) {
...
export function getBalanceAndSupplyData(balances : any) {
  ...
  for (let i = 0; i < keys.length; i++) {
    const key = keys[i];
    (balanceData as any)[key] = balances[i * propsLength];
    (supplyData as any)[key] = balances[i * propsLength + 1];
  }
...
export function getDepositBalanceData(depositBalances: any) {
  ...
  for (let i = 0; i < keys.length; i++) {
    const key = keys[i];
    (data as any)[key] = depositBalances[i];
  }
...
export function getVestingData(vestingInfo: any): RawVestingData | undefined {
  ...
  for (let i = 0; i < keys.length; i++) {
    ...
    (data as any)[key + "PairAmount"] = data[key]!.pairAmount;
    (data as any)[key + "VestedAmount"] = data[key]!.vestedAmount;
    (data as any)[key + "EscrowedBalance"] = data[key]!.escrowedBalance;
    (data as any)[key + "ClaimSum"] = data[key]!.claimedAmounts + data[key]!.claimable;
    (data as any)[key + "Claimable"] = data[key]!.claimable;
    (data as any)[key + "MaxVestableAmount"] = data[key]!.maxVestableAmount;
    (data as any)[key + "AverageStakedAmount"] = data[key]!.averageStakedAmount;
  }
...
export function importImage(name:any) {
