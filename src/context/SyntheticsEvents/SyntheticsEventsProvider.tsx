...
export function SyntheticsEventsProvider({
	...
	const { glvAndGmMarketsData, marketTokensAddressesString } = useMemo(() => {
		...
		const marketTokensAddressesString =
			Object.keys(glvAndGmMarketsData).join("-");
	...
	const [withdrawalStatuses, setWithdrawalStatuses] =
		useState<WithdrawalStatuses>({});
	const [shiftStatuses, setShiftStatuses] = useState<ShiftStatuses>({});

	const { tokensBalancesUpdates, setTokensBalancesUpdates } =
		useTokensBalancesUpdates();
	const [approvalStatuses, setApprovalStatuses] = useState<ApprovalStatuses>(
		{},
	);

	const [pendingPositionsUpdates, setPendingPositionsUpdates] =
		useState<PendingPositionsUpdates>({});
	...
	eventLogHandlers.current = {
		OrderCreated: (eventData: EventLogData, txnParams: EventTxnParams) => {
			...
			const data: OrderCreatedEventData = {
				...
				shouldUnwrapNativeToken:
					eventData.boolItems.items.shouldUnwrapNativeToken,
			...
			const metricId = isSwapOrderType(data.orderType)
				? getSwapOrderMetricId(data)
				: getPositionOrderMetricId(data);
			...
			if (order && marketsInfoData) {
				...
				if (pendingPositionKey) {
					setPendingPositionsUpdates((old) =>
						setByKey(old, pendingPositionKey!, undefined),
					);
				}
			}
		},
		...
		GlvDepositCreated: (
			...
			const depositData: GLVDepositCreatedEventData = {
				...
				initialLongTokenAddress:
					eventData.addressItems.items.initialLongToken,
				initialShortTokenAddress:
					eventData.addressItems.items.initialShortToken,
				longTokenSwapPath:
					eventData.addressItems.arrayItems.longTokenSwapPath,
				shortTokenSwapPath:
					eventData.addressItems.arrayItems.shortTokenSwapPath,
				initialLongTokenAmount:
					eventData.uintItems.items.initialLongTokenAmount,
				initialShortTokenAmount:
					eventData.uintItems.items.initialShortTokenAmount,
				initialMarketTokenAmount:
		...
		GlvDepositExecuted: (
			...
			if (depositStatuses[key]?.data) {
				const metricId = getGLVSwapMetricId(
					depositStatuses[key].data! as GLVDepositCreatedEventData,
				);
		...
		WithdrawalCreated: (
			...
			const data: WithdrawalCreatedEventData = {
				...
				minLongTokenAmount:
					eventData.uintItems.items.minLongTokenAmount,
				minShortTokenAmount:
					eventData.uintItems.items.minShortTokenAmount,
				updatedAtBlock: eventData.uintItems.items.updatedAtBlock,
				executionFee: eventData.uintItems.items.executionFee,
				callbackGasLimit: eventData.uintItems.items.callbackGasLimit,
				shouldUnwrapNativeToken:
		...
		PositionIncrease: (
			...
			const data: PositionIncreaseEvent = {
				...
				longTokenFundingAmountPerSize:
					eventData.intItems.items.longTokenFundingAmountPerSize,
				shortTokenFundingAmountPerSize:
					eventData.intItems.items.shortTokenFundingAmountPerSize,
				collateralDeltaAmount:
					eventData.intItems.items.collateralDeltaAmount,
				isLong: eventData.boolItems.items.isLong,
			...
			if (
				!isMarketOrderType(data.orderType) ||
				!orderStatuses[data.orderKey]
			) {
				let text = "";
				...
				const collateralToken = getToken(
					chainId,
				...
				pushSuccessNotification(chainId, text, {
					transactionHash: txnParams.transactionHash,
				});
			}
		},
		...
		PositionDecrease: (
			...
			if (
				...
				if (data.sizeDeltaUsd == 0n) {
					...
					const orderTypeLabel = isLiquidationOrderType(
						data.orderType,
					)
						? t`Liquidated`
						: t`Decreased`;
				...
				if (isLiquidationOrderType(data.orderType)) {
					pushErrorNotification(chainId, text, {
						transactionHash: txnParams.transactionHash,
					});
				} else {
					pushSuccessNotification(chainId, text, {
	...
	const contextState: SyntheticsEventsContextType = useMemo(() => {
		return {
			...
			async setPendingPosition(update: PendingPositionUpdate) {
				setPendingPositionsUpdates((old) =>
					setByKey(old, update.positionKey, update),
				);
			},

			setOrderStatusViewed(key: string) {
				setOrderStatuses((old) =>
					updateByKey(old, key, { isViewed: true }),
				);
			},
	...
	return (
		<SyntheticsEventsContext.Provider value={contextState}>
			{children}
		</SyntheticsEventsContext.Provider>
	);
}
