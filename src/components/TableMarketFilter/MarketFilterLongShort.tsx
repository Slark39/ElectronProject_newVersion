...
import {
	createSelector,
	useSelector,
} from "context/SyntheticsStateContext/utils";
...
export function MarketFilterLongShort({
	...
	const marketsOptions = useMemo<
		Group<MarketFilterLongShortItemData>[]
	>(() => {
			...
			strippedOpenPositions = positions.map((position) => ({
				...
				data: {
					marketAddress: position.market
						.marketTokenAddress as Address,
					direction: position.isLong ? "long" : "short",
					collateralAddress:
						position.collateralTokenAddress as Address,
				},
			}));
		}
		...
		(props: { item: MarketFilterLongShortItemData }) => {
			...
			return (
				<>
					...
					<div className="inline-flex items-center">
						{longOrShortText && (
							<span className="mr-3">{longOrShortText}</span>
						)}
						<span>{indexName}</span>
						<span className="subtext">[{poolName}]</span>
					</div>
