...
import CrossIconComponent from "img/cross.svg";
...
export default function SearchInput({
	...
	return (
		<div
			className={cx(
				"relative cursor-pointer rounded-4 border border-gray-800 p-0",
				className,
			)}
		>
			<input
				...
				className={cx(
					"block w-full bg-scroll bg-[12px_center] bg-no-repeat placeholder-slate-100",
					{
						"bg-[length:20px_20px] py-10 pl-40 pr-34 text-16":
							size === "m",
						"bg-[length:15px] py-[8.5px] pl-34 pr-30 text-14 ":
							size === "s",
					},
				)}
				style={STYLE}
			...
			<button
				className={cx(
					"group absolute bottom-0 right-0 top-0 flex items-center",
					{
						"pr-8": size === "m",
						"pr-4": size === "s",
					},
