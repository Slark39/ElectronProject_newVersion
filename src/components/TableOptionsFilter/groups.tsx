...
import Checkbox from "components/Checkbox/Checkbox_v2";
...
function GroupComponent<T>({
	...
	return (
		<div
			key={group.groupName}
			className="TableOptionsFilter-group group/group"
		>
			{showGroupToggle ? (
				<div
				...
				>
					<Checkbox
						isPartialChecked={
							group.isSomethingSelected &&
							!group.isEverythingSelected
						}
				...
				<div className="TableOptionsFilter-group-name">
					{group.groupName}
				</div>
			)}
		...
		.map((group, groupIndex) => {
			const items = !search.trim()
				...
				: group.items.filter((item, itemIndex) =>
						matchedItems.some(
							(matchedItem) =>
								matchedItem.id === `${groupIndex}_${itemIndex}`,
						),
					);
			...
			if (value && multiple) {
				...
				isEverythingSelected = group.items.every((item) =>
					castedValue.some((selectedItem) =>
						isEqual(selectedItem, item.data),
					),
				);
