...
export function TableOptionsFilter<T>({
	...
	const isActive =
		(multiple ? Boolean(value?.length) : Boolean(value)) || forceIsActive;
	...
	const showGroupToggle = Boolean(
		multiple && !disableGroupSelection && isGrouped,
	);
		...
		(newItem: T) => {
			if (multiple) {
				...
				if (ensuredValue.some((pair) => isEqual(pair, newItem))) {
					onChange(
						ensuredValue.filter((pair) => !isEqual(pair, newItem)),
					);
				} else {
	...
	const getIsSelected = useCallback(
		(item: T) => {
			if (multiple) {
				const ensuredValue = value || EMPTY_ARRAY;
				return ensuredValue.some((selectedPair) =>
					isEqual(selectedPair, item),
				);
			}
					...
					ensuredValue.filter(
						(pair) =>
							!group.items.some((item) =>
								isEqual(pair, item.data),
							),
					),
				);
			} else {
				onChange(
					ensuredValue.concat(group.items.map((item) => item.data)),
				);
			}
		},
		...
		>
			...
			<div className="TableOptionsFilter-options">
				<div className="TableOptionsFilter-clear" onClick={handleClear}>
					<Trans comment="Button to clear the filter selection">
						Clear selection
					</Trans>
				</div>
