...
function TradersStats({
	...
	const { getCurrentData, currentPage, setCurrentPage, pageCount } =
		usePagination(
			"TradersStats",
	...
	const currentTierDiscount = getSharePercentage(
		traderTier,
		discountShare,
		totalRebate,
	);
								...
								<Tooltip
									...
									renderContent={() => (
										<p className="text-white">
											<Trans>
												You will receive a{" "}
												{currentTierDiscount}% discount
												on opening and closing fees.
											</Trans>
											<br />
											<br />
											<Trans>
												For trades on V1, this discount
												will be airdropped to your
												account every Wednesday. On V2,
												discounts are applied
												automatically and will reduce
												your fees when you make a trade.
											</Trans>
											{((discountShare ?? 0) > 0 && (
												<>
													<br />
					...
					tooltipContent={
						<>
							<StatsTooltipRow
								label={t`V1 Arbitrum`}
								value={getUSDValue(
									arbitrumData?.traderReferralTotalStats
										.v1Data.volume,
								)}
							/>
							<StatsTooltipRow
								...
								value={getUSDValue(
									avalancheData?.traderReferralTotalStats
										.v1Data.volume,
								)}
							/>
							{isDevelopment() && (
								<StatsTooltipRow
									...
									value={getUSDValue(
										fujiData?.traderReferralTotalStats
											.v1Data.volume,
									)}
								/>
							)}
							...
							<StatsTooltipRow
								label={t`Total`}
								value={getUSDValue(total?.traderVolume)}
							/>
						</>
					}
				/>
				<ReferralInfoCard
					...
					tooltipContent={
						<>
							...
							<StatsTooltipRow
								label={t`Total`}
								value={getUSDValue(total?.discountUsd)}
							/>
						</>
					}
				/>
					...
					>
						<TableScrollFadeContainer>
							<table className="w-full min-w-max">
								...
								<tbody>
									{currentDiscountDistributions.map(
										(rebate) => {
									...
									{currentDiscountDistributions.length > 0 &&
										currentDiscountDistributions.length <
											DEFAULT_PAGE_SIZE && (
