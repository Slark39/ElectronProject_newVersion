...
import BottomTablePagination from "components/Pagination/BottomTablePagination_v2";
...
function AffiliatesStats({
	...
	const [isAddReferralCodeModalOpen, setIsAddReferralCodeModalOpen] =
		useState(false);
	...
	const allReferralCodes = affiliateReferralCodesStats?.map((c) =>
		c.referralCode.trim(),
	);
	const finalAffiliatesTotalStats = useMemo(
		() =>
			recentlyAddedCodes
				?.filter(isRecentReferralCodeNotExpired)
				.reduce((acc, cv) => {
	...
	const currentRebatePercentage = getSharePercentage(
		tierId,
		BigInt(discountShare ?? 0n),
		totalRebate,
		true,
	);
	...
	const totalClaimableRewardsUsd = useMemo(() => {
		...
		return getTotalClaimableAffiliateRewardsUsd(
			marketsInfoData,
	...
	return (
		<div className="referral-body-container">
			<div className="referral-stats">
				<ReferralInfoCard
					value={String(
						currentReferralsData?.affiliateTotalStats
					...
					tooltipContent={
						<>
							<StatsTooltipRow
								...
								value={
									arbitrumData.affiliateTotalStats
							...
							<StatsTooltipRow
								...
								value={
									avalancheData.affiliateTotalStats
										.registeredReferralsCount
								}
								showDollar={false}
							/>
							...
							<StatsTooltipRow
								label={t`Total`}
								value={total?.registeredReferralsCount}
								showDollar={false}
							/>
						</>
					}
					...
					tooltipContent={
						<>
							<StatsTooltipRow
								label={t`V1 Arbitrum`}
								value={getUSDValue(
									arbitrumData?.affiliateTotalStats.v1Data
										.volume,
								)}
							/>
							<StatsTooltipRow
								...
								value={getUSDValue(
									avalancheData?.affiliateTotalStats.v1Data
										.volume,
								)}
							/>
							{isDevelopment() && (
								<StatsTooltipRow
									...
									value={getUSDValue(
										fujiData?.affiliateTotalStats.v1Data
											.volume,
									)}
								/>
							)}
							<StatsTooltipRow
								label={t`V2 Arbitrum`}
								value={getUSDValue(
									arbitrumData?.affiliateTotalStats.v2Data
										.volume,
								)}
							/>
							<StatsTooltipRow
								...
								value={getUSDValue(
									avalancheData?.affiliateTotalStats.v2Data
										.volume,
								)}
							/>
							{isDevelopment() && (
								<StatsTooltipRow
									...
									value={getUSDValue(
										fujiData?.affiliateTotalStats.v2Data
											.volume,
									)}
								/>
							)}
							...
							<StatsTooltipRow
								label={t`Total`}
								value={getUSDValue(total?.affiliateVolume)}
							/>
						</>
					}
				/>
					...
					<TableScrollFadeContainer>
						<table className="w-full">
							...
							<tbody>
										...
										>
											<TableTd data-label="Referral Code">
												<div className="table-referral-code">
													...
													<ReferralCodeWarnings
														allOwnersOnOtherChains={
															stat?.allOwnersOnOtherChains
														}
													/>
												</div>
											</TableTd>
											...
											<TableTd data-label="Traders Referred">
								...
								{currentAffiliatesData.length > 0 &&
									currentAffiliatesData.length <
										DEFAULT_PAGE_SIZE && (
								...
								<tbody>
									{currentRebateData.map((rebate, index) => {
										...
										if (
											...
											rebate.typeId ===
											RebateDistributionType.Claim
										) {
											rebateType = t`V2 Claim`;
										}

										const amountsByTokens =
											rebate.tokens.reduce(
												(acc, tokenAddress, i) => {
													let token;
													try {
														token = getToken(
										...
										const totalUsd =
											rebate.amountsInUsd.reduce(
												(acc, usdAmount, i) => {
													if (
														usdAmount == 0n &&
														rebate.amounts[i] != 0n
													) {
														tokensWithoutPrices.push(
															rebate.tokens[i],
														);
													}
										...
										return (
											<TableTr
												...
												bordered={false}
											>
												<TableTd data-label="Date">
													{formatDate(
														rebate.timestamp,
													)}
												</TableTd>
												<TableTd data-label="Type">
													{rebateType}
												</TableTd>
