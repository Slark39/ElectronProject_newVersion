...
type Props = {
	...
	supportedResolutions:
		| typeof SUPPORTED_RESOLUTIONS_V1
		| typeof SUPPORTED_RESOLUTIONS_V2;
	oraclePriceDecimals?: number;
	visualMultiplier?: number;
};

export default function TVChartContainer(props: Props) {
	const {
		symbol,
		chainId,
		chartLines,
	...
	useEffect(() => {
		...
		//   "hello chartToken",
		//   chartToken,
		//   "hello supportedResolutions",
		//   supportedResolutions,
		//   "hello oraclePriceDecimals",
		//   oraclePriceDecimals,
		//   "hello visualMultiplier",
		//   visualMultiplier
		// );
		console.log(props);
		if (
			chartToken &&
			"maxPrice" in chartToken &&
			chartToken.minPrice !== undefined
		) {
	...
	useEffect(() => {
		...
		const widgetOptions = {
			...
			symbol:
				symbolRef.current &&
				getSymbolName(symbolRef.current, visualMultiplier), // Using ref to avoid unnecessary re-renders on symbol change and still have access to the latest symbol
			datafeed: datafeed,
			...
			disabled_features: isMobile
				? defaultChartProps.disabled_features.concat(
